...
| `-ModelFilter` | *(empty)* | Optional substring filter on model IDs |
...
The `.openrouter_key` file and `start.bat` (which typically contains the key in plain text) are excluded from version control via `.gitignore`. A safe pattern for local use:

```powershell
# Save once
"sk-or-v1-..." | Set-Content .openrouter_key

# Load and run
$env:OPENROUTER_API_KEY = Get-Content .openrouter_key
.\openrouter-proxy.ps1
```
